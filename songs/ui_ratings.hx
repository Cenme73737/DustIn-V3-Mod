...
function postCreate() {
    ...
    if (FlxG.save.data.hitb == true) {
        hitWindow *= .7;
    }else {
        hitWindow *= 1.5; // UH OH STRICTER HIT REG
    }
    ...
    comboRatings = [
		new ComboRating(0, "保持决心", 0xFF44FFFF),
...
function onPlayerHit(_) {
    ...
    if (noteDiff > hitWindow * 0.5)
        ...
        _.rating = 'good';
    _.showSplash = true;//Options.splashesEnabled && !_.note.isSustainNote && _.rating == "sick";
    ...
    if (!FlxG.save.data.hudvis) return;
