...
function startVideo(name:String, ?leCallback:Void->Void, ?ext:String, ?usePath:Bool) {
    ...
    // 修改：在跳过提示文本中添加“空格”
    add(skipText = new FunkinText(-28, FlxG.height - 50 - 6, FlxG.width, "按住 回车/空格/鼠标左 键以跳过...").setFormat(Paths.font('8bit-jve.ttf'), 32, 0xffffffff, "right", FlxTextBorderStyle.OUTLINE, 0xff000000));
...
function update(elapsed:Float) if (vid != null) {
    // 修改：添加 FlxG.keys.pressed.SPACE，让空格键与回车、鼠标左键共用“按住跳过”逻辑
    if (!FlxG.mouse.pressed && !FlxG.keys.pressed.ENTER && !FlxG.keys.pressed.SPACE) {  // i cant use accept cuz we dont have acept_hold, only in dev still sob  - Nex
        holdTime = 0;
