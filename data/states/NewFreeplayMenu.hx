...
function create() {
    FlxG.save.data.mechanics = false;
    ...
    for (i => song in FreeplaySonglist.get().songs) {
        ...
        if (creditData != null) {
            for (j in 0...creditLabels.length) {
                var field = creditFields[j];
                var label = creditLabels[j];
                var color = creditColors[j];

                if (Reflect.hasField(creditData, field)) {
                    var textLabel = label;
                    var creditValue = Reflect.field(creditData, field);

                    var labelMargin:Float = 30;
                    var valueMargin:Float = 40;  

                    var labelTxt = new FunkinText(0, 0, boxBG.width, textLabel + ":", 35, false);
                    labelTxt.setFormat(Paths.font("fallen-Tdown.ttf"), 30, color);
                    ...
                    valueTxt.setFormat(Paths.font("fallen-Tdown.ttf"), 23, 0xFFFFFFFF);
            ...
            var noCreds = new FunkinText(0, 0, boxBG.width, "容器尚未准备好。", 28, false);
            noCreds.setFormat(Paths.font("fallen-Tdown.ttf"), 28);
