...
function postCreate() {
    ...
    titleLabel.size = 48;
    descLabel.size = FlxG.save.data.decf;//24
        ...
        if (menu.health != -1) {
            ...
            switch (menu.rawName) {
                case "optionsTree.gameplay-name":
                    ...
                    var opponentModeAllow:Checkbox = null;
                    menu.insert(1, opponentModeAllow = new Checkbox("OpponentModeAllowed", "开启/关闭 对手模式\n开启后可以使用原生对手模式(不兼容Botplay)\n切勿同时开启CoopAllowed\n只能在FreePlay里使用", "opp", null, FlxG.save.data));
                    opponentModeAllow.color = 0xFFEFC3CA;

                    var coopAllow:Checkbox = null;
                    menu.insert(2, coopAllow = new Checkbox("CoopAllowed", "开启/关闭 合作模式\n开启后可以使用原生合作模式(不兼容Botplay)\n切勿同时开启OpponentModeAllowed\n只能在FreePlay里使用", "coop", null, FlxG.save.data));
                    coopAllow.color = 0xFFFFD700;

                    menu.insert(3, middlescrollOption = new Checkbox("Middlescroll", "若勾选此项,按键布局将移到中间\n警告:可能与有些歌曲设置冲突,并可能产生巨大的BUG!", "middlescrollOption", null, FlxG.save.data));
                    menu.insert(4, noHitCheckbox = new Checkbox("No Hit Mode", "不要漏掉任何一个音符,否则你就输了!!!\n歌曲结束后获得经验(EXP)倍率调整(1倍倍率 -> 2倍倍率)", "nh", null, FlxG.save.data));
                    menu.insert(5, mechanicsHitCheckbox = new Checkbox("Mechanics", "启用/禁用 游戏玩法机制\n歌曲结束后获得经验(EXP)倍率调整(1倍倍率 -> 0.5倍倍率)", "mechanics", null, FlxG.save.data));

                    middlescrollOption.color = 0xFF8A50FF;
                    noHitCheckbox.color = 0xFFC9FEFF;
                    mechanicsHitCheckbox.color = 0xFF8CDBFF;

                    menu.members[7].locked = true; // lock naughtyness
