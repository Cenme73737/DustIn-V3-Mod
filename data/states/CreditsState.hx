...
function create() {
    ...
    shText.textField.antiAliasType = 10/*ADVANCED*/;
    ...
    roleText = newText(lineBox.x + 50, lineBox.y + 40, 400,  "职责", 50, true, FlxTextAlign.LEFT);
    roleInfo = newText(lineBox.x + 50, lineBox.y + 110, 400, devs[0].roles, 23, true, FlxTextAlign.LEFT);
    nameText = newText(0, 0, 300, devs[0].name, 40, true, FlxTextAlign.CENTER);
    spHeader = newText(0, 0, 400, "特别鸣谢", 60, true, FlxTextAlign.CENTER);
    liHeader = newText(0, 0, 400, "官方页面", 60, true, FlxTextAlign.CENTER);
    ...
    lineText.setFormat(Paths.font("8bit-jve.ttf"), 25, FlxColor.WHITE, FlxTextAlign.LEFT); //简介
...
function update(elapsed:Float) {
    ...
    if ((FlxG.keys.justPressed.ENTER || FlxG.keys.justPressed.SPACE) && !isAnimating) {
    ...
    if (infoVisible) {
        ...
        } else if (imageDisplay.alpha > 0 && (FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.A || FlxG.keys.justPressed.RIGHT || FlxG.keys.justPressed.D)) {
            // 修改：添加 A 键对应左键（左移）、D 键对应右键（右移）的判断
            var dir = (FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.A) ? -1 : 1;
            if (!isTransitioning) {
                devTransition(dir);
