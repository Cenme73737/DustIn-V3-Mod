...
function create() {
    ...
    if (!FlxG.save.data.dustinBeatEverything) _list.remove("画廊");
    ...
    background.alpha = 0;
    cheatText = new FunkinText(0, 20, FlxG.width, "", 20, true);
    cheatText.setFormat(Paths.font("8bit-jve.ttf"), 32, 0xFFFF0000, "center");
...
function update(elapsed:Float):Void {
    ...
    if (FlxG.keys.justPressed.E) {
        FlxG.save.data.dustinCash += 1225;
        showCheatText('+114514 经验值 (当前经验值: ' + FlxG.save.data.dustinCash + ')');
    }
    if (FlxG.keys.justPressed.R) {
        FlxG.save.data.dustinCash = 0;
        showCheatText("经验值已重置");
    }
    ...
    if ((firstIntro ? !intro : true) && (FlxG.keys.justPressed.ENTER || FlxG.mouse.justPressed)) {
        select();
    }
...
function select() {
    ...
    switch (_list[curSelected]) {
        case "故事模式": FlxG.switchState(new StoryMenuState());
        case "自由选曲": FlxG.switchState(new ModState("NewFreeplayMenu"));
        case "商店": FlxG.switchState(new ModState("ShopState"));
        case "画廊": FlxG.switchState(new ModState("gallery/GalleryState"));
        case "设置": FlxG.switchState(new OptionsMenu());
        case "名单": FlxG.switchState(new ModState("CreditsState"));
        default: trace('idk');
    }
}

function showCheatText(text:String) {
    cheatText.text = text;
